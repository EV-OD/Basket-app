...
import {
  ...
  setRefetch,
  deleteItemsByIds,
} from '../../stores/subscriptions/subscriptionsSlice';
...
const TableSampleSubscriptions = ({
  filterItems,
  setFilterItems,
  filters,
  showGrid,
  ...
  useEffect(() => {
    if (subscriptionsNotify.showNotification) {
      notify(
        subscriptionsNotify.typeNotification,
        subscriptionsNotify.textNotification,
      );
    }
  ...
  const handleViewAction = (id: string) => {
    router.push(`/subscriptions/subscriptions-view/?id=${id}`);
  };

  const handleCreateEventAction = ({ start, end }: SlotInfo) => {
    router.push(
      `/subscriptions/subscriptions-new?dateRangeStart=${start.toISOString()}&dateRangeEnd=${end.toISOString()}`,
    );
  };
