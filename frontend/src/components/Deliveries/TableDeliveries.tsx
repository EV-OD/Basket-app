...
import {
  ...
  setRefetch,
  deleteItemsByIds,
} from '../../stores/deliveries/deliveriesSlice';
import { useAppDispatch, useAppSelector } from '../../stores/hooks';
import { useRouter } from 'next/router';
import { Field, Form, Formik } from 'formik';
import { DataGrid, GridColDef } from '@mui/x-data-grid';
import { loadColumns } from './configureDeliveriesCols';
...
const TableSampleDeliveries = ({
  ...
  useEffect(() => {
    if (deliveriesNotify.showNotification) {
      notify(
        deliveriesNotify.typeNotification,
        deliveriesNotify.textNotification,
      );
    }
  }, [deliveriesNotify.showNotification]);
  ...
  const handleViewAction = (id: string) => {
    router.push(`/deliveries/deliveries-view/?id=${id}`);
  };

  const handleCreateEventAction = ({ start, end }: SlotInfo) => {
    router.push(
      `/deliveries/deliveries-new?dateRangeStart=${start.toISOString()}&dateRangeEnd=${end.toISOString()}`,
    );
  };
  ...
  return (
    <>
      ...
      {!showGrid && (
        <BigCalendar
          events={deliveries}
          showField={'pickup_date'}
          start-data-key={'pickup_date'}
          end-data-key={'delivery_date'}
...
export default TableSampleDeliveries;
