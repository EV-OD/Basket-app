...
module.exports = class UsersDBApi {
  ...
  static async findBy(where, options) {
    ...
    output.deliveries_delivery_personnel =
      await users.getDeliveries_delivery_personnel({
        transaction,
      });

    output.orders_consumer = await users.getOrders_consumer({
      transaction,
    });

    output.products_farmer = await users.getProducts_farmer({
      transaction,
    });

    output.reviews_consumer = await users.getReviews_consumer({
      transaction,
    });

    output.subscriptions_consumer = await users.getSubscriptions_consumer({
      transaction,
    });
