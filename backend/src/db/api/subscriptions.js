...
module.exports = class SubscriptionsDBApi {
  static async create(data, options) {
    ...
    await subscriptions.setConsumer(data.consumer || null, {
      transaction,
    });
  ...
  static async findBy(where, options) {
    ...
    output.consumer = await subscriptions.getConsumer({
      transaction,
    });
  ...
  static async findAll(filter, options) {
    ...
    let { rows, count } = options?.countOnly
      ? {
          rows: [],
          count: await db.subscriptions.count({
            where,
            include,
            distinct: true,
      ...
      : await db.subscriptions.findAndCountAll({
          where,
          include,
          distinct: true,
          limit: limit ? Number(limit) : undefined,
  ...
  static async findAllAutocomplete(query, limit) {
    ...
    if (query) {
      where = {
        [Op.or]: [
          { ['id']: Utils.uuid(query) },
          Utils.ilike('subscriptions', 'start_date', query),
        ],
      };
    }
